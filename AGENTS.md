...
- PC Health는 개인용 Windows PC의 인터넷 장애 진단과 드라이버 상태 확인을 다루는 Tauri + React + Rust 데스크톱 앱이다.
- 인터넷 장애 진단은 승인된 설계를 가진 `active` 영역이다. 드라이버 관리는 `planning` 영역이며 승인된 설계 전에는 구현하지 않는다.
...
1. `docs/superpowers/specs/2026-06-21-product-pivot-network-driver-design.md`, 관련 최신 spec·plan, `docs/harness/pc-health/team-spec.md`, 필요한 `.agents/skills/`를 먼저 읽는다.
...
- driver download·설치·rollback, reboot, 자동 권한 상승, Windows Update 설정 변경은 별도 설계와 명시적 승인 전까지 금지한다.
- 드라이버 `planning` 요청은 요구사항·설계에서 멈춘다.
