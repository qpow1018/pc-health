...
description: Use when reviewing pc-health lifecycle boundaries, network probes, incident classification, SQLite retention, permissions, or network safety risks.
---
...
- 제거된 제품 영역을 우회해 재도입하지 않는가?
- 제거된 성능 모니터링 구조를 우회해 재도입하지 않는가?

## 차단 조건

router·OS 설정 변경, packet capture, 자동 복구, 제거된 제품 영역 복구, 자동 권한 상승, 근거 없는 확정 판정은 차단한다.
