...
1. `docs/superpowers/specs/2026-06-26-network-only-product-scope-design.md`, team spec, 관련 최신 network spec·plan과 코드를 읽는다.
2. 요청을 제품 영역과 lifecycle로 분류하고 가정과 성공 기준을 적는다.
3. 네트워크 구현은 `network-diagnostics-specialist`, UI·배치·안전 경계는 필요한 specialist만 선택한다.
4. 가장 작은 변경을 구현하고 변경 표면에 맞춰 검증한다.
5. 외부 요청, 로컬 보존, 권한 또는 시스템 변경이 관련되면 `qa-safety-reviewer`로 검토한다.

## 차단

- 새 제품 결정 없이 성능 provider, helper, sensor contract나 dashboard를 복구하지 않는다.
- 새 제품 결정 없이 제거된 제품 영역을 복구하지 않는다.
