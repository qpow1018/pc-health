...
| QA 안전 검토 | `.agents/skills/qa-safety-reviewer/SKILL.md` | lifecycle, 외부 요청, 보존 정책, 권한과 네트워크 안전 경계 검토 |
...
4. 현재 구조에 맞춰 수술적으로 구현한다. 제거된 제품 영역을 되살리지 않는다.
...
요청: 장애 시 router를 재시작하거나 Windows 네트워크 설정을 자동 수정한다.
